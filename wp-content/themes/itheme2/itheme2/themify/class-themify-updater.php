...
class Themify_Upgrader_Skin extends WP_Upgrader_Skin {
	...
	function __construct($args = array()) {
		...
		$defaults = array(	'type' 	=> '',	'login' => '',	'url' 	=> '', 'theme' => '',
							...
							'nonce' => '',	'title' => __('更新主题', 'themify'), 'cookies' => null );
	...
	function after() {
		...
		if ( !empty($this->upgrader->result['destination_name']) &&
			...
			$update_actions['preview'] = '<a href="' . $preview_link . '" class="thickbox thickbox-preview" title="' . esc_attr(sprintf(__('Preview &#8220;%s&#8221;', 'themify'), $name)) . '">' . __('预览', 'themify') . '</a>';

			$update_actions['activate'] = '<a href="' . $activate_link .  '" class="activatelink" title="' . esc_attr( sprintf( __('Activate &#8220;%s&#8221;', 'themify'), $name ) ) . '">' . __('激活', 'themify') . '</a>';
		...
		$update_actions['themes_page'] = '<a href="' . self_admin_url('admin.php?page=themify') . '" title="' . __('返回Themify面板', 'themify') . '" target="_parent">' . __('Return to Themify Panel', 'themify') . '</a>';
...
class Themify_Upgrader extends WP_Upgrader {
	...
	function upgrade_strings() {
		...
		$this->strings['up_to_date'] = __('主题是最新版本。', 'themify');

		$this->strings['no_package'] = __('更新包不可用。', 'themify');

		$this->strings['downloading_package'] = __('从下载更新<span class="code">%s</span>&#8230;', 'themify');

		$this->strings['unpack_package'] = __('解压缩升级&#8230;', 'themify');

		$this->strings['remove_old'] = __('删除旧版本的主题&#8230;', 'themify');

		$this->strings['remove_old_failed'] = __('无法删除旧的主题.', 'themify');

		$this->strings['process_failed'] = __('主题更新失败.', 'themify');

		$this->strings['process_success'] = __('主题更新成功.', 'themify');
	...
	function tcopydir($from, $to, $skip_list = array() ) {
		...
		foreach ( (array) $dirlist as $filename => $fileinfo ) {
			...
			if ( in_array( $filename, $skip_list ) ){
				...
				echo '<p><strong>' . sprintf(__('跳过 %s', 'themify'), $filename) .  '</strong></p>';
			...
			elseif ( $wp_filesystem->exists($to . $filename) && 'f' == $fileinfo['type'] ){
				...
				echo '<p>' . sprintf(__('删除 %s', 'themify'), $filename) . '</p>';
			...
			echo '<p>' . sprintf(__('复制 %s', 'themify'), $filename) . '</p>';
			...
			if ( 'f' == $fileinfo['type'] ) {
				...
				if ( ! $wp_filesystem->copy($from . $filename, $to . $filename, true, FS_CHMOD_FILE) ) {
					...
					if ( ! $wp_filesystem->copy($from . $filename, $to . $filename, true, FS_CHMOD_FILE) )
						...
						return new WP_Error('copy_failed', __('无法复制文件.', 'themify'), $to . $filename);
				...
				if ( !$wp_filesystem->is_dir($to . $filename) ) {
					...
					if ( !$wp_filesystem->mkdir($to . $filename, FS_CHMOD_DIR) )
						...
						return new WP_Error('mkdir_failed', __('无法创建目录.', 'themify'), $to . $filename);
	...
	function download_url( $url, $timeout = 400 ) {
		...
		if ( ! $url )
			...
			return new WP_Error('http_no_url', __('提供的网址无效.', 'themify'));
		...
		if ( ! $tmpfname )
			...
			return new WP_Error('http_no_file', __('无法创建临时文件.', 'themify'));
