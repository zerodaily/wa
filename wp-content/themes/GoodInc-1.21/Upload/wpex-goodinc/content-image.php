...
if ( is_singular() ) { ?>
	...
	<?php if( wpex_get_data( 'blog_single_thumbnail', '1' ) == '1' && has_post_thumbnail() ) { ?>
        <div id="post-thumbnail">
            ...
            <a href="<?php echo wp_get_attachment_url( get_post_thumbnail_id() ); ?>" title="<?php echo the_title(); ?>" id="post-thumbnail-lightbox-link" class="wpex-lightbox wpex-fadein"></a>
        </div><!-- #post-thumbnail -->
	<?php } ?>
    ...
    <article id="post-<?php the_ID(); ?>" <?php post_class('post-entry clr'); ?>>  
        <?php if( has_post_thumbnail() ) {  ?>
            <div class="post-entry-thumbnail">
                <a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"><img src="<?php echo aq_resize( wp_get_attachment_url( get_post_thumbnail_id() ), wpex_img('entry_width'),  wpex_img('entry_height'),  wpex_img('entry_crop') ); ?>" alt="<?php echo the_title(); ?>" /></a>
		...
		<footer class="post-entry-footer clr">
			<div class="post-entry-date"><?php echo get_the_date(); ?></div>
